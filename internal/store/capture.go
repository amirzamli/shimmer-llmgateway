...
func insertRequest(ctx context.Context, tx *sql.Tx, rec *CaptureRecord, ts string) error {
	_, err := tx.ExecContext(ctx, `
		INSERT INTO requests (
			...
			usage_json, request_json, request_filtered_json, upstream_request_json,
		...
		nullBytes(rec.Usage), nullBytes(rec.RequestJSON), nullBytes(rec.RequestFilteredJSON), nullBytes(rec.UpstreamRequestJSON),
