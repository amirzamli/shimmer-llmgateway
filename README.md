...
- **Templates need no alias line.** A template's first instance auto-defaults its alias to the template name, so you can address a provider directly as `provider-name/model-name` (e.g. `openai/gpt-4o`) without configuring an alias. When a template has multiple instances, the later ones auto-default to `name-2`, `name-3`, … — address those as `provider-name-2/model-name`, etc.
